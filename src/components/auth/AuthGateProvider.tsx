    ...
    <AuthGateContext value={contextValue}>
      ...
      {modal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60" onClick={() => setModal(null)}>
          <div
            className="bg-surface border border-border rounded-lg p-6 w-full max-w-xs shadow-2xl"
          ...
          >
            ...
            <div className="flex flex-col gap-2">
              <Link
                ...
                className="w-full py-2 bg-surface border border-profit/40 rounded font-mono text-sm text-profit text-center uppercase tracking-wider hover:bg-profit/10 transition-colors"
              >
                Sign In
              </Link>
              <Link
                href={`/signup?redirectTo=${redirectTo}`}
                className="w-full py-2 bg-surface border border-border rounded font-mono text-sm text-text-secondary text-center uppercase tracking-wider hover:border-text-muted transition-colors"
              >
                Sign Up
              </Link>
            </div>
